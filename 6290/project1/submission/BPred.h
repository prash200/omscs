...
protected:
    std::map<InstID, std::pair<unsigned long long, unsigned long long> > predStatsMap; // Count hit and miss per instruction

    void updatePredStats(const Instruction *inst, PredType pred) {
        std::pair<unsigned long long, unsigned long long> predStats;
        if (predStatsMap.find(inst->currentID()) != predStatsMap.end()) {
            predStats = predStatsMap[inst->currentID()];
        } else {
            predStats = std::make_pair(0, 0);
        }
        ...
        predStatsMap[inst->currentID()] = predStats;
    }
    ...
    void reportPredStats(const char *type) {
        ...
        std::cout << (hits[3]/(hits[3]+misses[3]))*100 << std::endl << std::endl;

        double overallAccuracy = ((hits[0] + hits[1] + hits[2] + hits[3])/(hits[0]+misses[0]+hits[1]+misses[1]+hits[2]+misses[2]+hits[3]+misses[3]))*100;
        std::cout << "Overall accuracy for " << type << " branch predictor: " << overallAccuracy << " %. This number should match BPred generated by report.pl." << std::endl;
    }
public:
